...
const Input = ({filter, setFilter, breed, setBreed}) => {
   

    const renderFilters = () => {
        return ['random', 'breed'].map((fil, index) => {
            return (
                <div onClick={()=>setFilter(fil)} key = {index} className={fil === filter ? "default-btn mar-right-16" : "unselected-btn mar-right-16"}>
                        <h5>
                            {fil}
                        </h5>
                </div>
    ...
    const fetchBreeds = async () => {
        ...
        setBreeds(Object.keys(res.breeds).map(key=>{ return {value: key, label: key} } ));
    }
